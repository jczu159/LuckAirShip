...
public class LuckAirShipScan {
	...
	private void initialize() {
		...
		JButton btnNewButton = new JButton("測試擷取");
				...
				try {
					...
					while (!result) {
						try {
							...
							textArea.setText("目前分鐘數:" + realtime);
							System.out.println("目前分鐘數:" + realtime);

							if (Integer.valueOf(realtime) % 5 == 0) {

								String waitSec = formattedTextField_2.getText();
								textArea.setText("開始等待40秒");
								Thread.sleep(40 * 1000);
								textArea.setText("等待40秒完成");
								Screenshot.scanImg(setting);
